...
#+BEGIN_SRC racket
;; TODO: Well this is what SHOULD happen...
> (a-vat 'run
         (lambda ()
           (define-values (foo-promise foo-resolver)
             (spawn-promise-pair))
           (define-values (bar-promise bar-resolver)
             (spawn-promise-pair))
...
*TODO: Something is wrong here and this isn't working.*
*What is it?  Solve it before continuing.*
